...
is the enforcement: `_setup-guardian/reference/lessons.md` is the registry of what we
learned, and `_setup-guardian/scripts/doctor.py` is the doctor that refuses to let us
...
1. **Run the doctor and read the report.**
   ...
   ```
   python3 skills/setup-guardian/_setup-guardian/scripts/doctor.py
   ```
...
2. **Use `--fix` for the safe auto-fixes only.** The doctor rebuilds
   ...
   ```
   python3 skills/setup-guardian/_setup-guardian/scripts/doctor.py --fix
   ```
...
3. **`--watch` for scheduled runs.** Prints exactly one JSON object to
   ...
   ```
   0 9 * * * /usr/bin/python3 /path/to/agentkit/skills/setup-guardian/_setup-guardian/scripts/doctor.py --watch >> /var/log/agentkit-doctor.log 2>&1
   ```
...
- **Every repeat-mistake class gets a LESSON entry in
  `_setup-guardian/reference/lessons.md` PLUS a doctor check.** The registry is the
