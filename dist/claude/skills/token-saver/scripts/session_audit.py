...
def main(argv=None):
    ...
    ap.add_argument("--long", type=int, default=300, help="flag sessions above this many turns")
    ap.add_argument("--json", action="store_true")
    a = ap.parse_args(argv)

    files = sorted(glob.glob(os.path.join(a.root, "*", "*.jsonl")), key=os.path.getmtime)[-a.last:]
    ...
    print(f"{'project':32} {'turns':>6} {'startup':>8} {'fresh_in':>10} {'cache_read':>12} {'output':>9}")
    for r in rows:
        flag = "  <- long: /clear or split" if r["long"] else ""
        print(f"{r['project']:32} {r['turns']:6} {r['startup']:8} {r['fresh_in']:10} "
