...
`_token-saver/reference/patterns.md` for before/after command examples and
`_token-saver/scripts/estimate.py` to size a file before you touch it.

## Procedure

1. **Size before you read.** Run `_token-saver/scripts/estimate.py` on candidate
   files. It reports lines/bytes/estimated tokens and a read strategy:

   ```
   python3 _token-saver/scripts/estimate.py src/app.py logs/server.log --budget 8000
   ```
...
- One-off need? Skip the skill entirely: `python3 _token-saver/scripts/estimate.py --budget <n> <files>` tells you whether the read fits.
- Don't read `_token-saver/reference/patterns.md` whole — jump to the one section matching your task.
