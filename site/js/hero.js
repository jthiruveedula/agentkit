...
const WRAP = document.querySelector(".hero__scene-wrap");
const CARD = document.querySelector(".node-card");
const CARD_ICON = CARD?.querySelector(".node-card__icon use");
const CARD_NAME = CARD?.querySelector(".node-card__name");
const CARD_DESC = CARD?.querySelector(".node-card__desc");
...
function initScene() {
  ...
  const nodes = TOOLS.map((tool, i) => {
    ...
    const pos = new THREE.Vector3(Math.cos(angle) * radius, tilt, Math.sin(angle) * radius);
  ...
  function showCard(tool) {
    if (!CARD) return;
    CARD_NAME.textContent = tool.name;
